...
public class CrazyStreams {
    ...
    public int getNumOfLettersInFirstAndLastNames() {
        int firstNameSum = accounts.stream()
                .map(Account::getFirstName)
                .mapToInt(String::length)
                .sum();

        int lastNameSum = accounts.stream()
                .map(Account::getLastName)
                .mapToInt(String::length)
                .sum();

        return firstNameSum + lastNameSum;
    }
    ...
    public BigDecimal getBalanceByEmail(String email) {
        return accounts.stream()
                ...
                .findAny().orElseThrow(()-> new EntityNotFoundException("Cannot find Account by email=" + email));
    }
    ...
    public Map<Month, String> groupCommaSeparatedFirstNamesByBirthdayMonth() {
        ...
        Map<Month, List<Account>> collect3 = accounts.stream()
                .collect(Collectors.groupingBy(account -> account.getBirthday().getMonth()));

        return collect3.entrySet().stream()
